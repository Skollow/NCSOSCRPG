...
namespace Engine.Services
{
    public static class CombatService
    {
        ...
        public static bool CriticalHit(LivingEntity attacker, LivingEntity target)
        {
            ...
            return RandomNumberGenerator.NumberBetween(0, 100) <= 5 + totalOffset;
        }

        public static Combatant EscapeOpponent(Player player, Monster opponent)
        {
            // Formula is: ((Dex(player)^2 - Dex(monster)^2)/10) + Random(-10/10)
            // For dexterity values from 3 to 18, this should produce an offset of +/- 41.5
            int playerDexterity = player.Dexterity * player.Dexterity;
            ...
            return RandomNumberGenerator.NumberBetween(0, 100) <= 80 + totalOffset
                       ? Combatant.Player
                       : Combatant.Opponent;
        }
        public static Combatant EscapedButHarmed(Player player, Monster opponent)
        {
            // Formula is: ((Dex(player)^2 - Dex(monster)^2)/10) + Random(-10/10)
            // For dexterity values from 3 to 18, this should produce an offset of +/- 41.5
            int playerDexterity = player.Dexterity * player.Dexterity;
            ...
            return RandomNumberGenerator.NumberBetween(0, 100) <= 60 + totalOffset
                       ? Combatant.Player
                       : Combatant.Opponent;
        }
    }
